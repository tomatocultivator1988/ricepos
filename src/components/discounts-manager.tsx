...
export function DiscountsManager() {
  ...
  function openEditDialog(discount: Discount) {
    ...
    setForm({ name: discount.name, type: discount.type, value: discount.value })
    setDialogOpen(true)
  }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      const body = { name: form.name.trim(), type: form.type, value: form.value }
      let res
      if (editingItem) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-foreground">Discounts</h2>
      ...
      <div className="grid grid-cols-1 gap-3 lg:hidden">
        {loading ? (
          ...
          <div className="rounded-2xl border-2 border-amber-300/60 bg-gold-200/90 p-8 text-center text-stone-500 shadow-md">No discounts found</div>
        ) : (
          filtered.map((discount) => (
            <div key={discount.id} onClick={() => openEditDialog(discount)} className="rounded-2xl border-2 border-amber-300/60 bg-gold-200/90 p-4 shadow-md cursor-pointer hover:border-gold-400/50">
              <div className="flex justify-between items-start mb-2">
                <span className="font-bold text-stone-800 text-sm">{discount.name}</span>
                <span className="text-amber-600 font-extrabold">{formatValue(discount)}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="capitalize text-stone-500">{discount.type} · {discount.is_active ? 'Active' : 'Inactive'}</span>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-transparent hover:bg-transparent">
              ...
              <TableHead className="text-xs font-semibold uppercase text-amber-600">Type</TableHead>
              <TableHead className="text-xs font-semibold uppercase text-amber-600">Value</TableHead>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={5} className="py-8 text-center text-stone-500">Loading...</TableCell>
              </TableRow>
            ) : filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="py-8 text-center text-stone-500">No discounts found</TableCell>
              </TableRow>
            ) : (
              filtered.map((discount) => (
                <TableRow key={discount.id} className="cursor-pointer transition-colors hover:bg-gold-200/50" onClick={() => openEditDialog(discount)}>
                  <TableCell className="font-medium text-stone-800">{discount.name}</TableCell>
                  <TableCell className="capitalize text-stone-700">{discount.type}</TableCell>
                  <TableCell className="font-mono tabular-nums text-stone-700">{formatValue(discount)}</TableCell>
                  <TableCell>
                    {discount.is_active ? (
        ...
        <DialogContent className="rounded-2xl bg-gold-200/90 border-amber-300/60 text-stone-800 p-5">
          ...
          <div className="grid gap-4 py-2">
            <div className="grid gap-2">
              ...
              <Input id="dm-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="bg-gold-100" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="dm-type">Type *</Label>
                <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v ?? "percentage" })}>
                  <SelectTrigger id="dm-type" className="bg-gold-100">
              ...
              <div className="grid gap-2">
                ...
                <Input id="dm-value" type="number" step="0.01" min="0" value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} className="bg-gold-100" />
              </div>
            </div>
          </div>
          <DialogFooter>
